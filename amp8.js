...
var c=new CPU(ROM); c.nv.set(new Uint8Array(fs.readFileSync('PCF8570_abbild.bin')));
c.xram.set(new Uint8Array(fs.readFileSync('D310_abbild.bin')).slice(0,0x8000)); c.pc=0;
var puf=[]; c.log=[];
var ds=CPU.prototype.ds, st=CPU.prototype.step;
c.ds=function(a,v){ if(a===0x99) puf.push(v&255); ds.call(this,a,v); };
c.step=function(){ var op=this.rom[this.pc];
  if(op===0xF0){var d=this.dptr(); if(d>=0x8000){var n=(d>>8)&15; if(n&&puf.length)this.log.push([n,puf.slice()]); if(n)puf=[];}}
  st.call(this); };
for(var i=0;i<9000000;i++)c.step();
var tog=0;
function taste(k){ tog^=1; c.log=[]; c.key(k,tog); for(var i=0;i<2200000;i++)c.step(); }
...
function zeile(t){
  ...
  console.log(t.padEnd(10)+' 56h/57h='+h(c.ram[0x56])+h(c.ram[0x57])+'  1Ch='+h(c.ram[0x1C])+
   ' 1Eh='+h(c.ram[0x1E])+'  AC-Feld='+acf()+'  STR9='+(s9?s9[1].map(h).join(''):'--')+
   ' STR7='+(s7?s7[1].map(h).join(' '):'--'));
}
taste(0x2A);            // Knopf frei
taste(0x11);            // Sinus
taste(0x19); taste(0x19);   // AC anwaehlen und einschalten
zeile('AC ein');
for(var r=1;r<=8;r++){ c.log=[]; c.rad(1,1); for(i=0;i<2400000;i++)c.step(); zeile('Rad '+r); }
